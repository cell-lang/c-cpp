...
void raw_int_col_iter_init(UNARY_TABLE *master_table, RAW_INT_COL *column, RAW_INT_COL_ITER *iter) {
  unary_table_iter_init(master_table, &iter->iter);
  iter->array = column->array;
}

bool raw_int_col_iter_is_out_of_range(RAW_INT_COL_ITER *iter) {
  unary_table_iter_is_out_of_range(&iter->iter);
}

uint32 raw_int_col_iter_get_idx(RAW_INT_COL_ITER *iter) {
  return unary_table_iter_get(&iter->iter);
}

int64 raw_int_col_iter_get_value(RAW_INT_COL_ITER *iter) {
  uint32 idx = unary_table_iter_get(&iter->iter);
  return iter->array[idx];
}

void raw_int_col_iter_move_forward(RAW_INT_COL_ITER *iter) {
