...
const uint32 OVERFLOW_TABLE_SIZE_2_BLOCK_MIN_COUNT   = 3;
const uint32 OVERFLOW_TABLE_SIZE_4_BLOCK_MIN_COUNT   = 4;
const uint32 OVERFLOW_TABLE_SIZE_8_BLOCK_MIN_COUNT   = 7;
const uint32 OVERFLOW_TABLE_SIZE_16_BLOCK_MIN_COUNT  = 13;
const uint32 OVERFLOW_TABLE_HASHED_BLOCK_MIN_COUNT   = 13;

////////////////////////////////////////////////////////////////////////////

static uint32 overflow_table_get_capacity(uint32 tag) {
  assert(tag >= SIZE_2_BLOCK & tag <= SIZE_16_BLOCK);
  assert(SIZE_2_BLOCK == 1 | SIZE_16_BLOCK == 4);
  return 2 << tag;
}

static uint64 overflow_table_linear_block_handle(uint32 tag, uint32 index, uint32 count) {
  return pack(pack_tag_payload(tag, index), count);
}

static uint64 overflow_table_size_2_block_handle(uint32 index, uint32 count) {
  assert(get_tag(index) == 0);
  assert(count >= OVERFLOW_TABLE_SIZE_2_BLOCK_MIN_COUNT & count <= 4);
  return pack(pack_tag_payload(SIZE_2_BLOCK, index), count);
}

static uint64 overflow_table_size_4_block_handle(uint32 index, uint32 count) {
  assert(get_tag(index) == 0);
  assert(count >= OVERFLOW_TABLE_SIZE_4_BLOCK_MIN_COUNT & count <= 8);
  return pack(pack_tag_payload(SIZE_4_BLOCK, index), count);
}

static uint64 overflow_table_size_8_block_handle(uint32 index, uint32 count) {
  assert(get_tag(index) == 0);
  assert(count >= OVERFLOW_TABLE_SIZE_8_BLOCK_MIN_COUNT & count <= 16);
  return pack(pack_tag_payload(SIZE_8_BLOCK, index), count);
}

// static uint64 overflow_table_size_16_block_handle(uint32 index, uint32 count) {
//   assert(get_tag(index) == 0);
//   assert(count >= OVERFLOW_TABLE_SIZE_16_BLOCK_MIN_COUNT & count <= 32);
//   return pack(pack_tag_payload(SIZE_16_BLOCK, index), count);
// }

static uint64 overflow_table_hashed_block_handle(uint32 index, uint32 count) {
  assert(get_tag(index) == 0);
...
static uint32 overflow_table_min_count(uint32 tag) {
  if (tag == SIZE_2_BLOCK)
    return OVERFLOW_TABLE_SIZE_2_BLOCK_MIN_COUNT;

  if (tag == SIZE_4_BLOCK)
    return OVERFLOW_TABLE_SIZE_4_BLOCK_MIN_COUNT;

  if (tag == SIZE_8_BLOCK)
    return OVERFLOW_TABLE_SIZE_8_BLOCK_MIN_COUNT;

  assert(tag == SIZE_16_BLOCK | tag == HASHED_BLOCK);
  assert(OVERFLOW_TABLE_SIZE_16_BLOCK_MIN_COUNT == OVERFLOW_TABLE_HASHED_BLOCK_MIN_COUNT);

  return OVERFLOW_TABLE_SIZE_16_BLOCK_MIN_COUNT; // Same as OVERFLOW_TABLE_HASHED_BLOCK_MIN_COUNT
}

////////////////////////////////////////////////////////////////////////////

static uint64 overflow_table_insert_2_block(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value, STATE_MEM_POOL *mem_pool) {
  ...
  return overflow_table_size_2_block_handle(block_idx, 3);
}

static uint64 overflow_table_insert_into_hashed_block(ARRAY_MEM_POOL *, uint32, uint32, uint32, STATE_MEM_POOL *);

static uint64 overflow_table_insert_with_linear_block(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value, STATE_MEM_POOL *mem_pool) {
  ...
  for (uint32 i=0 ; i < end ; i++) {
    ...
    if (slot_high == EMPTY_MARKER) {
      ...
      return overflow_table_linear_block_handle(tag, block_idx, count + 1);
    }
  }

  uint32 capacity = overflow_table_get_capacity(tag);
  ...
  if (tag != SIZE_16_BLOCK) {
    ...
    return overflow_table_linear_block_handle(tag + 1, new_block_idx, count + 1);
  }
  ...
  for (uint32 i=0 ; i < 16 ; i++) {
    ...
    uint64 slot = slots[block_idx + i];
    uint64 tmp_handle = overflow_table_insert_into_hashed_block(array_pool, hashed_block_idx, 2 * i, get_low_32(slot), mem_pool);
    assert(get_count(tmp_handle) == 2 * i + 1);
    assert(get_payload(get_low_32(tmp_handle)) == hashed_block_idx);
    tmp_handle = overflow_table_insert_into_hashed_block(array_pool, hashed_block_idx, 2 * i + 1, get_high_32(slot), mem_pool);
    assert(get_count(tmp_handle) == 2 * (i + 1));
    assert(get_payload(get_low_32(tmp_handle)) == hashed_block_idx);
  }
  ...
  return overflow_table_insert_into_hashed_block(array_pool, hashed_block_idx, 32, value, mem_pool);
}

static uint64 overflow_table_insert_into_hashed_block(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 count, uint32 value, STATE_MEM_POOL *mem_pool) {
  uint64 *slots = array_pool->slots;
  ...
  if (low == EMPTY_MARKER) {
    ...
    return overflow_table_hashed_block_handle(block_idx, count + 1);
  }
  ...
  if (tag == INLINE_SLOT) {
    if (value == low)
      return overflow_table_hashed_block_handle(block_idx, count);
    ...
    if (value == high)
      return overflow_table_hashed_block_handle(block_idx, count);
    uint64 handle = overflow_table_insert_2_block(array_pool, pack(clipped(low), clipped(high)), clipped(value), mem_pool);
  ...
  uint64 handle;
  if (tag == HASHED_BLOCK)
    handle = overflow_table_insert_into_hashed_block(array_pool, get_payload(low), get_count(slot), clipped(value), mem_pool);
  else
    handle = overflow_table_insert_with_linear_block(array_pool, slot, clipped(value), mem_pool);
...
static uint64 overflow_table_insert_unique_into_hashed_block(ARRAY_MEM_POOL *, uint32, uint32, uint32, STATE_MEM_POOL *);

static uint64 overflow_table_insert_unique_with_linear_block(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value, STATE_MEM_POOL *mem_pool) {
  assert(get_tag(get_low_32(handle)) >= SIZE_2_BLOCK & get_tag(get_low_32(handle)) <= SIZE_16_BLOCK);
  ...
  for (uint32 i=0 ; i < 16 ; i++) {
    ...
    uint64 slot = slots[block_idx + i];
    uint64 tmp_handle = overflow_table_insert_unique_into_hashed_block(array_pool, hashed_block_idx, 2 * i, get_low_32(slot), mem_pool);
    assert(get_count(tmp_handle) == 2 * i + 1);
    assert(get_payload(get_low_32(tmp_handle)) == hashed_block_idx);
    tmp_handle = overflow_table_insert_unique_into_hashed_block(array_pool, hashed_block_idx, 2 * i + 1, get_high_32(slot), mem_pool);
    assert(get_count(tmp_handle) == 2 * (i + 1));
    assert(get_payload(get_low_32(tmp_handle)) == hashed_block_idx);
  }
  ...
  // Adding the new value
  return overflow_table_insert_unique_into_hashed_block(array_pool, hashed_block_idx, 32, value, mem_pool);
}

static uint64 overflow_table_insert_unique_into_hashed_block(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 count, uint32 value, STATE_MEM_POOL *mem_pool) {
  uint64 *slots = array_pool->slots;
  ...
  uint64 handle;
  if (tag == HASHED_BLOCK)
    handle = overflow_table_insert_unique_into_hashed_block(array_pool, get_payload(low), get_count(slot), clipped(value), mem_pool);
  else
    handle = overflow_table_insert_unique_with_linear_block(array_pool, slot, clipped(value), mem_pool);
...
static uint64 overflow_table_shrink_linear_block(ARRAY_MEM_POOL *array_pool, uint32 tag, uint32 block_idx, uint32 count) {
  uint64 *slots = array_pool->slots;
  ...
  if (tag == SIZE_4_BLOCK) {
    ...
    return overflow_table_size_2_block_handle(size_2_block_idx, count);
  }

  if (tag == SIZE_8_BLOCK) {
    assert(count == 6);
    array_mem_pool_release_8_block_upper_half(array_pool, block_idx);
    return overflow_table_size_4_block_handle(block_idx, count);
  }

  assert(tag == SIZE_16_BLOCK);
  assert(count == 12);
  array_mem_pool_release_16_block_upper_half(array_pool, block_idx);
  return overflow_table_size_8_block_handle(block_idx, count);
}

static uint64 overflow_table_copy_and_release_block(ARRAY_MEM_POOL *array_pool, uint64 handle, uint64 state, uint32 least_bits) {
...
static uint64 overflow_table_shrink_hashed_block(ARRAY_MEM_POOL *array_pool, uint32 block_idx) {
  assert(OVERFLOW_TABLE_HASHED_BLOCK_MIN_COUNT == 13);
  ...
  for (uint32 i=0 ; i < 6 ; i++)
    state = overflow_table_copy_and_release_block(array_pool, saved_slots[i], state, i);

  uint32 end_idx = block_idx + 6;
  for (uint32 i=6 ; get_low_32(state) < end_idx ; i++)
    state = overflow_table_copy_and_release_block(array_pool, target_slots[i], state, i);
  ...
  array_mem_pool_release_16_block_upper_half(array_pool, block_idx);
  return overflow_table_size_8_block_handle(block_idx, 12);
}

////////////////////////////////////////////////////////////////////////////////

static uint64 overflow_table_delete_from_linear_block(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value) {
  uint64 *slots = array_pool->slots;
  ...
  if (value == last_low | value == last_high) {
    ...
    if (count == overflow_table_min_count(tag))
      return overflow_table_shrink_linear_block(array_pool, tag, block_idx, count - 1);
    else
      return overflow_table_linear_block_handle(tag, block_idx, count - 1);
  }
...
static uint64 overflow_table_delete_from_hashed_block(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 count, uint32 value) {
  uint64 *slots = array_pool->slots;
  ...
  assert(count >= OVERFLOW_TABLE_HASHED_BLOCK_MIN_COUNT);

  // The value has actually been deleted. Shrinking the block if need be
  if (count > OVERFLOW_TABLE_HASHED_BLOCK_MIN_COUNT)
    return overflow_table_hashed_block_handle(block_idx, count - 1);
  else
    return overflow_table_shrink_hashed_block(array_pool, block_idx);
}
...
static bool overflow_table_linear_block_contains(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 count, uint32 value) {
  uint64 *slots = array_pool->slots;
  uint32 end = (count + 1) / 2;
  for (uint32 i=0 ; i < end ; i++) {
    uint64 slot = slots[block_idx + i];
...
static bool overflow_table_hashed_block_contains(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 value) {
  uint64 *slots = array_pool->slots;
  ...
  if (tag != HASHED_BLOCK)
    return overflow_table_linear_block_contains(array_pool, sub_block_idx, get_count(slot), clipped_value);
  else
    return overflow_table_hashed_block_contains(array_pool, sub_block_idx, clipped_value);
}
...
static uint32 overflow_table_linear_block_value_offset(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 count, uint32 value) {
  uint64 *slots = array_pool->slots;
  uint32 end = (count + 1) / 2;
  for (uint32 i=0 ; i < end ; i++) {
...
static uint32 overflow_table_hashed_block_value_offset(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 value) {
  uint64 *slots = array_pool->slots;
  ...
  if (tag != HASHED_BLOCK)
    return overflow_table_linear_block_value_offset(array_pool, sub_block_idx, get_count(slot), clipped_value);
  else
    return overflow_table_hashed_block_value_offset(array_pool, sub_block_idx, clipped_value);
}

////////////////////////////////////////////////////////////////////////////////

static uint32 overflow_table_copy_hashed_block(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 *dest, uint32 offset, uint32 shift, uint32 least_bits) {
  uint64 *slots = array_pool->slots;
  ...
  for (uint32 i=0 ; i < 16 ; i++) {
    ...
    if (low != EMPTY_MARKER) {
      ...
      if (tag == INLINE_SLOT) {
        ...
        target_idx = overflow_table_copy_hashed_block(array_pool, get_payload(low), dest, target_idx, subshift, slot_least_bits);
      }
      else {
...
static uint32 overflow_table_copy_hashed_block_range(ARRAY_MEM_POOL *array_pool, uint32 block_idx, uint32 first, uint32 *dest, uint32 capacity, uint32 shift, uint32 least_bits) {
  uint64 *slots = array_pool->slots;
    ...
    if (low != EMPTY_MARKER) {
      ...
      if (tag == INLINE_SLOT) {
        ...
        if (passed + count > first) {
          ...
          uint32 newly_written = overflow_table_copy_hashed_block_range(array_pool, get_payload(low), subfirst, dest + written, capacity - written, subshift, slot_least_bits);
...
static void overflow_table_hashed_block_insert_reversed(ARRAY_MEM_POOL *array_pool, uint32 key, uint32 block_idx, uint32 shift, uint32 least_bits, ONE_WAY_BIN_TABLE *target, STATE_MEM_POOL *mem_pool) {
  uint64 *slots = array_pool->slots;
  ...
  for (uint32 i=0 ; i < 16 ; i++) {
    ...
    if (low != EMPTY_MARKER) {
      ...
      if (tag == INLINE_SLOT) {
        ...
        overflow_table_hashed_block_insert_reversed(array_pool, key, get_payload(low), subshift, slot_least_bits, target, mem_pool);
      }
      else {
        uint32 subblock_idx = get_payload(low);
        uint32 count = get_count(slot);
...
uint64 overflow_table_insert(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value, STATE_MEM_POOL *mem_pool) {
  ...
  if (tag == 0)
    return overflow_table_insert_2_block(array_pool, handle, value, mem_pool);

  if (tag == HASHED_BLOCK)
    return overflow_table_insert_into_hashed_block(array_pool, get_payload(low), get_count(handle), value, mem_pool);

  return overflow_table_insert_with_linear_block(array_pool, handle, value, mem_pool);
}
...
uint64 overflow_table_insert_unique(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value, STATE_MEM_POOL *mem_pool) {
  ...
  if (tag == HASHED_BLOCK)
    return overflow_table_insert_unique_into_hashed_block(array_pool, get_payload(low), get_count(handle), value, mem_pool);

  return overflow_table_insert_unique_with_linear_block(array_pool, handle, value, mem_pool);
}
...
uint64 overflow_table_delete(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value) {
  ...
  if (tag == HASHED_BLOCK)
    return overflow_table_delete_from_hashed_block(array_pool, get_payload(low), get_count(handle), value);
  else
    return overflow_table_delete_from_linear_block(array_pool, handle, value);
}
...
bool overflow_table_contains(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value) {
  ...
  if (tag != HASHED_BLOCK)
    return overflow_table_linear_block_contains(array_pool, block_idx, get_count(handle), value);
  else
    return overflow_table_hashed_block_contains(array_pool, block_idx, value);
}
...
uint32 overflow_table_value_offset(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 value) {
  ...
  if (tag != HASHED_BLOCK)
    return overflow_table_linear_block_value_offset(array_pool, block_idx, get_count(handle), value);
  else
    return overflow_table_hashed_block_value_offset(array_pool, block_idx, value);
}
...
void overflow_table_copy(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 *dest, uint32 offset) {
  ...
  if (tag != HASHED_BLOCK) {
    ...
    overflow_table_copy_hashed_block(array_pool, block_idx, dest, offset, 0, 0);
}
...
UINT32_ARRAY overflow_table_range_copy(ARRAY_MEM_POOL *array_pool, uint64 handle, uint32 first, uint32 *dest, uint32 capacity) {
  ...
  if (tag != HASHED_BLOCK) {
    ...
    result.size = overflow_table_copy_hashed_block_range(array_pool, block_idx, first, dest, capacity, 0, 0);
    result.array = dest;
  }
...
void overflow_table_insert_reversed(ARRAY_MEM_POOL *array_pool, uint32 key, uint64 handle, ONE_WAY_BIN_TABLE *target, STATE_MEM_POOL *mem_pool) {
  ...
  if (tag != HASHED_BLOCK) {
    ...
    overflow_table_hashed_block_insert_reversed(array_pool, key, block_idx, 0, 0, target, mem_pool);
}
